...
<!-------------------------------------------------------------------------------------------->	
			...
			{
				if (form2.pay[0].checked)
				{
					if (!form2.card.value) {
						alert("카드종류를 선택하세요.");	form2.card.focus();	return;
					}
					if (!form2.account.value || form2.account.value.length!=16) {
						alert("카드번호를 입력하세요.");	form2.account.focus();	return;
					}
			...
			<c:forEach items="${ map.list }" var="list" varStatus="status">
				<tr bgcolor="#FFFFFF">
					...
					<td align="center"><font color="#464646">${list.cartPrice * list.num}</font> 원</td>
				</tr>
				</c:forEach>
				<tr>
			...
			<!-- form2 시작  -->
			<form name="form2" method="post"action="${pageContext.servletContext.contextPath }/user/order_ok1">

			<input type="hidden" name="product_no"   value="${map.productvo.no}">
			<input type="hidden" name = "product_num" value = "${map.productvo.num }"/> 
			...
			<input type="hidden" name="orderName"   value="${map.buyvo.orderName }">
			<input type="hidden" name="orderphone"  value="${map.buyvo.orderphone }">
			<input type="hidden" name="orderEmail"  value="${map.buyvo.orderEmail }">
			<input type="hidden" name="orderAddr"   value="${map.buyvo.orderAddr }">
			
			<input type="hidden" name="totalprice"   value="${total + 2500 }">
			<input type="hidden" name="user_no"   value="${authUser.no }">
			<input type="hidden" name="prono"   value="${map.productvo.no}">
					...
					<td align="center" width="560">
						...
						<table width="560" border="0" cellpadding="0" cellspacing="0" class="cmfont">
							<tr height="25">
								...
								<td width="20"><b>:</b></td>
								<td width="390">
									<input type="radio" name="pay" value ="카드" onclick="javascript:PaySel(0);" checked>카드 &nbsp;
									<input type="radio" name="pay" value = "무통장" onclick="javascript:PaySel(1);">무통장
								</td>
							</tr>
						</table>
					...
					<td align="center" width="560">
						<table width="560" border="0" cellpadding="0" cellspacing="0" class="cmfont">
							<tr height="25">
								...
								<td width="20"><b>:</b></td>
								<td width="390">
									<select name="card" class="cmfont1">
							...
							<tr height="25">
								...
								<td width="20"><b>:</b></td>
								<td width="390">
									<input type="text" name="account" size="16" maxlength="16" value="111" class="cmfont1"> 
							...
							<tr height="25">
								...
								<td width="390">
									<select name="halbu" class="cmfont1">
										...
										<option value="9개월">9 개월</option>
										<option value="12개월">12 개월</option>
									</select>
								</td>
							</tr>
						</table>
							...
							<tr height="25">
								...
								<td width="390">
									<select name="bank_kind"  class="cmfont1" disabled>
										...
										<option value="국민은행">국민은행 92-04646-77777</option>
										<option value="신한은행">신한은행 110-347-746761</option>
									</select>
								</td>
							</tr>
							<tr height="25">
